                ...
                if ($this->errors) {
                    ...
                    if ($offset1 >= $max1 AND $offset2 >= $max2 AND $offset3 >= $max3) {
                        ...
                        if ($max1) {
                            ...
                            $entityName = Mage1AttributeSet::model()->tableName();
                        }
                        if ($max2) { // has done with attribute sets
                            $max = $max2;
                            $offset = $offset2 + $this->limit;
                            $entityName = Mage1AttributeGroup::model()->tableName();
                        }
                        if ($max3) { // has done with attributes
                            $max = $max3;
                            $offset = $offset3 + $this->limit;
                            $entityName = Mage1Attribute::model()->tableName();
                        }
                        ...
                        //update result to respond
                        $rs['status'] = 'ok';
                        $rs['percent_up'] = UBMigrate::getPercentUp(3, $max, $this->limit);
    ...
    private function _migrateAttributeGroups($attributeGroups)
    {
        ...
        foreach ($attributeGroups as $attributeGroup) {
            ...
            if ($entityTypeCode1 == UBMigrate::PRODUCT_TYPE_CODE) {
                ...
                //check unique for attribute-set-id_attribute-group-code
                $checkCon = "attribute_set_id = {$attributeSetId2} AND attribute_group_code = '{$attributeGroupCode2}'";
                $found = Mage2AttributeGroup::model()->find($checkCon);
                if ($found) {
                    $attributeGroupCode2 = "{$attributeGroupCode2}-{$attributeGroup->attribute_set_id}";
                }
                //tab group code
        ...
        if ($entityTypeCode1 == UBMigrate::PRODUCT_TYPE_CODE) {
            ...
            if ($attributeSetting) {
                ...
                if (!$attributeSetting2) {
                    ...
                    if ($attribute->is_user_defined && $attributeSetting->is_configurable) {
