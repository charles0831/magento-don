    ...
    public function actionRun()
    {
        ...
        if ($check['allowed']) {
            try {
                ...
                $offset1 = $max1 = $offset2 = $max2 = $max3 = $offset3 = 0;
                ...
                if ($this->errors) {
                    ...
                    if ($offset1 >= $max1 AND $offset2 >= $max2 AND $offset3 >= $max3) { //has migrated all
                        ...
                        if ($max1) {
                            ...
                            $entityName = Mage1Website::model()->tableName();
                        }
                        if ($max2) { // has done with attribute sets
                            $max = $max2;
                            $offset = $offset2 + $this->limit;
                            $entityName = Mage1StoreGroup::model()->tableName();
                        }
                        if ($max3) { // has done with attributes
                            $max = $max3;
                            $offset = $offset3 + $this->limit;
                            $entityName = Mage1Store::model()->tableName();
                        }
