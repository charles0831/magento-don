    ...
    public function actionSetting()
    {
        ...
        if ($result['allowed']) {
            ...
            if (Yii::app()->request->isPostRequest) {
                ...
                if ($selectAll || $selectedCategoryIds) {
                    //make setting data to save
                    $settingData = [
                        'category_ids' => (!$selectAll) ? $selectedCategoryIds : [],
                        'select_all_category' => ($selectAll || sizeof($selectedCategoryIds) == $totalCategories) ? 1 : 0
                    ];
    ...
    public function actionRun()
    {
        ...
        if ($check['allowed']) {
            ...
            try {
                ...
                if ($selectedCategoryIds || $settingData['select_all_category']) {
                    //build condition to get data
                    if (!$settingData['select_all_category']) {
                ...
                if ($this->errors) {
                    ...
                    if ($offset >= $max) {
                        ...
                        //update result to respond
                        $rs['status'] = 'ok';
                        $rs['percent_up'] = UBMigrate::getPercentUp(1, $max, $this->limit);
                    ...
                    if ($attributeId2) {
                        ...
                        if ($table == 'catalog_category_entity_varchar') {
                            if ($this->_isRootCategory($entityId1)) {
                                ...
                                //uncomment below code lines to handle for bad data cases only
