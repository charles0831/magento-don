...
<h2 class="page-header">
    <?php echo Yii::t('frontend', 'Migration Settings'); ?> <i class="material-icons">keyboard_arrow_right</i>
    <?php echo Yii::t('frontend', $step->title); ?>
</h2>
...
<form class="frm-settings" role="form" method="post" action="<?php echo UBMigrate::getSettingUrl($step->sorder); ?>">
    <div id="step-content" class="step7">
        ...
        <div class="message tip">
            ...
            <p>
                <?php
                    echo Yii::t('frontend','- Other data refers to CMS Blocks, CMS Pages, Reviews, Rating, Product Tier Prices, Product Group Prices, Download Link Purchased, Tax Data, Email Templates...');
                    echo '<br/>';
                    echo Yii::t('frontend','- All Magento core & 3rd party widget shortcodes in the Content field of the CMS Blocks / Pages are ported to Magento 2, however it\'s required to manually configure the shortcodes at your end after migration.');
                ?>
            </p>
        </div>
        <div class="panel-group other-data-objects" role="tablist" aria-multiselectable="true">
            <div class="panel panel-default has-child">
                ...
                <div class="panel-body">
                    <div class="checkbox">
                        <label for="select-all" title="<?php echo Yii::t('frontend', 'Click here to select all.')?>">
                            <?php echo Yii::t('frontend', 'Select All');?>
                        </label>
                        <input type="checkbox" id="select-all"
                               name="select_all" <?php echo ($isMigrated) ? 'class="read-only" readonly="readonly" onclick="event.preventDefault();"' : ''; ?>
                               value="1" <?php echo (sizeof($selectedObjects) == sizeof($objects)) ? "checked" : ''; ?>
                    ...
                    <?php if ($objects): ?>
                        <ul class="list-group ver-nav">
                            <?php foreach ($objects as $object => $info): ?>
                                ...
                                $total = (in_array($object, array('tax_data', 'increment_ids', 'email_template_newsletter', 'cms')))
                                    ? null
                                    : UBMigrate::getTotalItemOfObject($object);
                                $suffix = (!is_null($total)) ?  " (". $total .")" : '';
                                $toolTip = ($object == 'catalog_product_entity_group_price'
                                    ? 'class="has-tooltip" data-toggle="tooltip" data-placement="right" title="'.Yii::t('frontend', 'All Product group prices will be convert to Product tier prices.').'"'
                                    : '');
                                ?>
                                <li class="list-group-item<?php echo ($hasChild) ? ' has-child' : ''; ?><?php echo ($disabled) ? ' read-only' : ''; ?>">
                                    <div class="list-group-item-heading">
                                        ...
                                        <?php endif; ?>
                                        <label for="object_<?php echo $object; ?>"
                                               class="checkbox-inline<?php echo ($disabled) ? ' read-only' : ''; ?>" <?php echo $toolTip; ?>>
                                        ...
                                        <ul class="list-group" style="display: none;">
                                            <?php foreach ($info['related'] as $childObject => $label) : ?>
                                                ...
                                                <li class="list-group-item<?php echo ($disabled) ? ' read-only' : ''; ?>">
                                                    <div class="list-group-item-heading">
                                                        <label class="checkbox-inline<?php echo ($disabled) ? ' read-only' : ''; ?>"
                                                               for="<?php echo $keyName ."_". $childObject; ?>">
