        ...
        <tr class="step <?php echo $step->getStepStatusClassCSS(); ?>">
            ...
            <td style="text-align: right;">
                <?php if ($step->status != UBMigrate::STATUS_PENDING AND $step->sorder != 1): ?>
                    <?php
                        $actionClass = ($step->status == UBMigrate::STATUS_SKIPPING
                            || $step->status == UBMigrate::STATUS_CLEANING) ? "btn-run disabled" : (($step->status == UBMigrate::STATUS_FINISHED) ? 'btn-run delta' : 'btn-run');
                    ?>
                ...
                <?php endif; ?>
                <a id="setting-step-<?php echo $step->sorder; ?>"
                   title="<?php echo Yii::t('frontend', 'Click to re-setting this step'); ?>"
                   href="<?php echo UBMigrate::getSettingUrl($step->sorder); ?>"
                   class="btn btn-default btn-setting">
                    <i class="material-icons">settings</i> <?php echo Yii::t('frontend', 'Setting'); ?>
                </a>
            </td>
        </tr>
        <?php endforeach; ?>
