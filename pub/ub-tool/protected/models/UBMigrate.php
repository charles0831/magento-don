...
class UBMigrate extends CActiveRecord
{
    ...
    const STATUS_CLEANING = 7; //Removing for migrated data was no longer exists in M1
    ...
    public function getStepStatusText()
    {
        $statuses = [
            ...
            self::STATUS_CLEANING => '<span class="step-status resetting">' . Yii::t('frontend', 'cleaning...') . '</span>',
    ...
    public function getStepStatusClassCSS()
    {
        $statuses = [
            ...
            self::STATUS_CLEANING => 'resetting',
    ...
    public static function getPercentUp($totalObjects, $max, $limit) {
        $percentUp = 0;
        if ($totalObjects && $max && $limit) {
            $totalSteps = self::getTotalStepCanRunMigrate();
            $percentOfOnceStep = (1 / $totalSteps) * 100;
            $percentUp = ($percentOfOnceStep / $totalObjects) / (ceil($max / $limit));
        }

        return $percentUp;
    }

    public static function resetMappingStates($stepIndex)
    {
        if ($stepIndex == 2) {
            $tables = [
        ...
        if ($tables) {
            ...
            foreach ($tables as $tableName) {
                $query = "UPDATE `{$tablePrefix}{$tableName}` SET `mapped` = '0'";
    ...
    /**
     * Todo: Delete the migrated Websites/Stores/Store views in M2 which has no longer existing in M1
     * @param bool|false $limit
     * @param string $tables
     * @return string
     * @throws CDbException
     */
    public function rmDeletedDataStep2($limit = false, $tables = '')
    {
        $status = 'fail';
        $condition = "mapped = 0";
        if (!empty($tables)) {
            $condition .= " AND entity_name IN ({$tables})";
        }
        $criteria = self::_getLogCriteria($limit, $condition);

        //get all entities was migrated in map log table of this step
        $models = MappingStep2::model()->findAll($criteria);
        if (!$models) { //has done with all
            $status = 'done';
        } else {
            $status = $this->_rmDeletedData($models);
        }
    ...
    /**
     * Todo: Delete the migrated Attributes and Attribute's values in M2 which has no longer existing in M1
     * @param bool|false $limit
     * @param string $tables
     * @return string
     * @throws CDbException
     */
    public function rmDeletedDataStep3($limit = false, $tables = '')
    {
        $status = 'fail';
        $condition = "mapped = 0";
        if (!empty($tables)) {
        ...
        //get all entities was migrated in map log table of this step
        $models = null;
        $models = MappingStep3AttributeOption::model()->findAll($criteria);
        if (!$models) {
            $models = MappingStep3Attribute::model()->findAll($criteria);
            if (!$models) {
                $models = MappingStep3::model()->findAll($criteria);
            }
        }
        if (!$models) { //has done with all
            $status = 'done';
    ...
    /**
     * Todo: Delete the migrated Categories in M2 which has no longer existing in M1
     * @param bool|false $limit
     * @param string $tables
     * @return string
     * @throws CDbException
     */
    public function rmDeletedDataStep4($limit = false, $tables = '')
    {
        $status = 'fail';
        $condition = "mapped = 0";
        if (!empty($tables)) {
        ...
        //get all entities was migrated in map log table of this step
        $models = null;
        $models = MappingStep4::model()->findAll($criteria);
        if (!$models) { //has done with all
            $status = 'done';
        } else {
            $status = $this->_rmDeletedData($models);
    ...
    /**
     * Todo: Delete the migrated Products and associated data in M2 which has no longer existing in M1
     * @param bool|false $limit
     * @param string $tables
     * @return string
     * @throws CDbException
     */
    public function rmDeletedDataStep5($limit = false, $tables = '')
    {
        $status = 'fail';
        $condition = "mapped = 0";
        if (!empty($tables)) {
        ...
        //get all entities was migrated in map log table of this step
        $models = null;
        $models = MappingStep5ProductOption::model()->findAll($criteria);
        if (!$models) {
            $models = MappingStep5ProductDownload::model()->findAll($criteria);
            if (!$models) {
                $models = MappingStep5::model()->findAll($criteria);
            }
        }
        if (!$models) { //has done with all
            $status = 'done';
    ...
    /**
     * Todo: Delete the migrated Customers and associated data in M2 which has no longer existing in M1
     * @param bool|false $limit
     * @param string $tables
     * @return string
     * @throws CDbException
     */
    public function rmDeletedDataStep6($limit = false, $tables = '')
    {
        $status = 'fail';
        $condition = "mapped = 0";
        if (!empty($tables)) {
        ...
        //get all entities was migrated in map log table of this step
        $models = null;
        $models = MappingStep6CustomerAddress::model()->findAll($criteria);
        if (!$models) {
            $models = MappingStep6::model()->findAll($criteria);
        }
        if (!$models) { //has done with all
            $status = 'done';
        } else {
    ...
    /**
     * Todo: Delete the migrated Sales Orders and associated data in M2 which has no longer existing in M1
     * @param bool|false $limit
     * @param string $tables
     * @return string
     * @throws CDbException
     */
    public function rmDeletedDataStep7($limit = false, $tables = '')
    {
        $status = 'fail';
        $condition = "mapped = 0";
        if (!empty($tables)) {
        ...
        //get all entities was migrated in map log table of this step
        $models = null;
        $models = MappingStep7InvoiceItem::model()->findAll($criteria);
        if (!$models) {
            $models = MappingStep7Invoice::model()->findAll($criteria);
            if (!$models) {
                $models = MappingStep7QuoteAddress::model()->findAll($criteria);
                if (!$models) {
                    $models = MappingStep7QuoteItem::model()->findAll($criteria);
                    if (!$models) {
                        $models = MappingStep7Quote::model()->findAll($criteria);
                        if (!$models) {
                            $models = MappingStep7OrderAddress::model()->findAll($criteria);
                            if (!$models) {
                                $models = MappingStep7OrderItem::model()->findAll($criteria);
                                if (!$models) {
                                    $models = MappingStep7Order::model()->findAll($criteria);
                                    if (!$models) {
                                        $models = MappingStep7::model()->findAll($criteria);
                                    }
                                }
                            }
                        }
    ...
    /**
     * Todo: Delete the migrated data objects in M2 which has migrated in step #8 and it was no longer existing in M1
     * @param bool|false $limit
     * @param string $tables
     * @return string
     * @throws CDbException
     */
    public function rmDeletedDataStep8($limit = false, $tables = '')
    {
        $status = 'fail';
        $condition = "mapped = 0";
        if (!empty($tables)) {
        ...
        //get all entities was migrated in map log table of this step
        $models = null;
        $models = MappingStep8Cms::model()->findAll($criteria);
        if (!$models) {
            $models = MappingStep8Subscriber::model()->findAll($criteria);
            if (!$models) {
                $models = MappingStep8Rating::model()->findAll($criteria);
                if (!$models) {
                    $models = MappingStep8ReviewSummary::model()->findAll($criteria);
                    if (!$models) {
                        $models = MappingStep8Review::model()->findAll($criteria);
                        if (!$models) {
                            $models = MappingStep8DownloadableLinkPurchased::model()->findAll($criteria);
                            if (!$models) {
                                $models = MappingStep8::model()->findAll($criteria);
                            }
                        }
                    }
                }
    ...
    protected static function _getLogCriteria($limit = -1, $condition = null)
    {
    ...
    protected function _rmDeletedData($models)
    {
        //update step's state
        ...
        foreach ($models as $model) {
            $needUpdate = true;
            if ($model->m1_id > 0 && $model->can_reset == self::RESET_YES) {
                $model1ClassName = str_replace("Mage2", "Mage1", $model->m2_model_class);

                //check exists in M1
                $found = $model1ClassName::model()->findByPk($model->m1_id);

                //We need to specific checking for migrated active quotes
                if ($model->entity_name == 'sales_flat_quote_active') {
                    $table1 = str_replace('_active', '', $model->entity_name);
                    $activeQuoteCriteria = New CDbCriteria();
                    $activeQuoteCriteria->condition = "entity_id = {$model->m1_id} AND is_active = 1";
                    $found = $model1ClassName::model()->find($activeQuoteCriteria);
                    $tip = '(active quote) ';
                } else {
                    $table1 = $model->entity_name;
                    $tip = '';
                }

                //if it was not existing in M1
                if (!$found) {
                    $model2 = new $model->m2_model_class();
                    //remove related migrated data item in M2
                    $rs = $model2->deleteAll("{$model->m2_key_field} = {$model->m2_id}");
                    if ($rs) {
                        //out put
                        $table2 = str_replace(['{{', '}}'], ['', ''], $model2->tableName());
                        $out = "Deleted M2 entity {$tip}({$model->m2_key_field} = {$model->m2_id}) in the '{$table2}' table";
                        $out .= " <=> M1 entity ({$model->m2_key_field} = {$model->m1_id}) in the '{$table1}' table";
                        //remove the mapping data record
                        if ($model->delete()) {
                            $needUpdate = false;
                            Yii::log($out, 'info', 'ub_data_migration');
                            echo "\n{$out}\n";
                        }
                    }
                } else {
                    $out = ".";
                    if (($i % 65) == 0 || $i == sizeof($models)) {
                        $out .= "\n";
                    }
                    echo $out;
                }
    ...
    {
        $status = 'fail';
        $criteria = self::_getLogCriteria($limit);

        //get all entities was migrated in map log table of this step
        $className = "MappingStep{$this->sorder}";
        $models = $className::model()->findAll($criteria);
        if ($models) {
            $status = $this->_deleteMigratedItems($models);
        } else { // has done
            /**
             * Special process for step #2:
             * Process to revert the original default website/default store and original default Root category ID
             */
            if ($this->sorder == 2) {
    ...
    /**
     * Todo: Delete all migrated data objects in step #5
     * @param bool|false $limit
     * @return string
     * @throws CDbException
     */
    public function resetDataStep5($limit = false)
    {
        ...
        //get all entities was migrated in mapping log tables of this step
        $models = MappingStep5ProductOption::model()->findAll($criteria);
        if (!$models) {
            $models = MappingStep5ProductDownload::model()->findAll($criteria);
            if (!$models) {
    ...
    protected function _resetStepStatus()
    {
    ...
    protected function _deleteMigratedItems($models)
    {
        //update step's status for current step
        if ($this->status != self::STATUS_RESETTING) {
            $this->updateStatus(self::STATUS_RESETTING);
        }
        //process to delete related data
        foreach ($models as $model) {
            if ($model->can_reset == self::RESET_YES) {
                //delete M2 entity was migrated
                $model2 = new $model->m2_model_class();
                $model2->deleteAll("{$model->m2_key_field} = {$model->m2_id}");
                /**
                 * because some tables has not constraint with the main table
                 * So we have to process delete for related data in these tables.
                 */
                if ($model->entity_name == 'catalogrule') {
                    Mage2CatalogruleProduct::model()->deleteAll("rule_id = {$model->m2_id}");
                } elseif ($model->entity_name == 'sales_flat_order') {
                ...
                } elseif ($model->entity_name == 'catalog_product_entity') {
                ...
                } elseif ($model->entity_name == 'catalog_category_entity') {
    ...
    public static function convertDateTimeToUtc($strDateTime, $timeZone)
    {
        $dateTime1 = new \DateTime($strDateTime, new \DateTimeZone($timeZone));
        return $dateTime1->setTimezone(new \DateTimeZone('UTC'))->format("Y-m-d H:i:s");
    }
}
